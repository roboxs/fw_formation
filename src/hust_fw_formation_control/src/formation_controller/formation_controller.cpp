...
//重载函数
void FORMATION_CONTROLLER:: set_formation_type(int id,int swarm_shape,int swarm_size,int swarm_num)
 {
    Eigen::MatrixXf speration= FORMATION_CONTROLLER::get_formation_separation(swarm_shape,swarm_size,swarm_num);
    formation_offset.xb=speration(id,0);
    formation_offset.yb=speration(id,1);
    formation_offset.zb=speration(id,2);
...
{
    if ((abs(leader_states.global_vel_x) > 0.1) ||
        (abs(leader_states.global_vel_y) > 0.1) ||
        (abs(leader_states.relative_alt) > 0.1))
    {
        led_in_fly = true;
    }
    else
