...
#include "../fixed_wing_lib/my_controller.hpp"
...
public:
    ...
    /*得到发射原点*/
    void get_launch_point(double lat,double lon,double alt);
    /*控制器*/
    void controller();
private:
    /**
   *编队控制器外函数，变量（组）
   */
    /*****存放     当前经纬度与原点经纬度之差后转换到ned距离N-E距离差****/
    Point NE_Distance;
    ...
    PID_CONTROLLER thrust_ctl;
    PID_CONTROLLER pitch_ctl;
    /* 重置内部控器标志量 */
    bool rest_speed_pid{false};
    bool rest_my_contoller{false};
    ...
    /*float leader_vel_prev;*/
    float leader_vel_prev;
    float leader_vel_prev_1;
    /**
   * TECS函数，变量（组）
   */
    ...
    bool rest_pid_h{false};
     bool rest_pid_p{false};
    /* 纵向速度有效标志位 */
    bool vz_valid{false};
   ...
   */
    ...
    void cal_plane_states();
    /*领机经维，从机经纬，方向向量*/
    Point led_pos0;
    Point led_pos_cur;
    Point fol_pos_cur;
    Vec vector_fol_led;//从机与领机向量
    Vec led_dir_unit;//领机方向向量
    Vec fw_dir_unit;//从机方向向量
    /*领机的起始经纬高，后期自己界面设置发射原点*/
    double led_lat0{0.0};
    double led_lon0{0.0};
    double led_alt0{0.0};
    ...
    float VZg_led{0.0};
    /*******控制量********/
    float K_x,Kd_x,Ka_x,Ua_x,Uv_x,Uv_prev_x,Ar_x,Vprev_x;
    float K_y,Kd_y,Ka_y,Ua_y,Uv_y,Uv_prev_y,Ar_y,Vprev_y;
    float K_z,Kd_z,Ka_z,Ua_z,Uv_z,Uv_prev_z,Ar_z,Vprev_z;
    float Uh_z_prev;
