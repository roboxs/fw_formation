...
Point cov_lat_long_2_m(Point O, Point P)
{ //参考点是O点，lat，long
    Point delt;
    double lat1 = O.x;
    double lon1 = O.y;
    ...
    double n_distance = deg_2_rad(lat2 - lat1) * CONSTANTS_RADIUS_OF_EARTH; //涉及到ned是向北增加，且纬度向北也增加

    double r_at_ref1 = cos(deg_2_rad(lat1)) * CONSTANTS_RADIUS_OF_EARTH;
...
double CalculateHeading(double lat1,double lat2,double lon1,double lon2 )
{
     double a = (90 - lat2) * PI / 180;
    double b = (90 -lat1) * PI / 180;
    double AOC_BOC = (lon2-lon1) * PI / 180;
    ...
    double res = 0;
    if (lon2> lon1 && lat2 > lat1) res = A;
    else if (lon2 > lon1 && lat2 <lat1) res = 180 - A;
    else if (lon2 <lon1 &&  lat2 < lat1) res = 180 - A;
    else if (lon2 <lon1 &&  lat2 > lat1) res = 360 + A;
    else if (lon2 >lon1 &&  lat2 == lat1) res = 90;
    else if (lon2 <lon1 &&  lat2 == lat1) res = 270;
    else if (lon2 ==lon1 &&  lat2 > lat1) res = 0;
    else if (lon2 ==lon1 &&  lat2 < lat1) res = 180;
