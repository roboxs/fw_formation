...
private:
    ...
    float integ_throttle_max{1};

    float integ_throttle_min{0};

    float integ_pitch_max{10};

    float integ_pitch_min{-10};
...
public:
    ...
    //抗饱和积分 pid
    float pid_anti_saturated(float input_val, bool use_integ, bool use_diff,string s);
...
float PID_CONTROLLER::pid_anti_saturated(float input_val, bool use_integ, bool use_diff,string s)
{
    ...
    if(s=="pitch")
    {
        integ_max=integ_pitch_max;
        integ_min=integ_pitch_min;
    }
    else if(s=="throttle")
    {
        integ_max=integ_throttle_max;
        integ_min=integ_throttle_min;
    }
   // cout<<"input_val:"<<input_val<<endl;
    cal_time_interval();
